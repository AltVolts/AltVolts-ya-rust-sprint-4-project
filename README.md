...
## Настройка окружения
- Проверяем корректность сборки плагинов через запуск тестов: собираем проект и запускаем тесты. Библиотеки плагинов будут находиться в ```/target/debug``` и ```/target/debug/deps```
```shell
  cargo build
```
...
- Собираем оптимизированную сборку плагинов - полученные библиотеки плагинов по умолчанию будут находиться в ```/target/release```
```shell
 cargo build_mirror
```
